...
```yaml
# ci.yml
name: agw-${{ matrix.os }}-${{ matrix.arch }}
# becomes:
name: agx-${{ matrix.os }}-${{ matrix.arch }}

# release.yml
asset_name: agw-macos-arm64
# becomes:
...
- Line 72: `name: agw-${{ matrix.os }}-${{ matrix.arch }}`
  - AGX: `name: agx-${{ matrix.os }}-${{ matrix.arch }}`
  - AGW: `name: agw-${{ matrix.os }}-${{ matrix.arch }}`
- Line 75: `target/${{ matrix.target }}/release/agw`
  - AGX: `target/${{ matrix.target }}/release/agx`
  - AGW: `target/${{ matrix.target }}/release/agw`
- Line 76: `target/${{ matrix.target }}/release/agw.exe` (keep for Windows if needed)

### `pr-checks.yml`
Search and replace:
- Line 16: `if [[ ! "$PR_TITLE" =~ ^AGW-[0-9]+: ]]; then`
...
### `CICD_SETUP.md`
Search and replace:
- **All instances** of `AGW` → `AGX` or `AGW`
- **All instances** of `agw` → `agx` or `agw`
...
for file in ci.yml pr-checks.yml release.yml; do
  sed "s/agw/$REPO_NAME/g; s/AGW/$REPO_CODE/g" \
    ../agw/.github/workflows/$file > .github/workflows/$file
done

cp ../agw/.github/CICD_SETUP.md .github/
sed -i '' "s/agw/$REPO_NAME/g; s/AGW/$REPO_CODE/g" .github/CICD_SETUP.md

cp ../agw/rust-toolchain.toml .
...
- [ ] All `agw`/`AGW` references replaced
...
**Symptom:** Release creates `agw` binary instead of `agx`/`agw`
...
- Check AGW repository for reference implementation
