...
**Artifacts Generated:**
```
agw-macos-arm64.tar.gz      + SHA256
agw-macos-x86_64.tar.gz     + SHA256
agw-linux-x86_64.tar.gz     + SHA256
agw-linux-arm64.tar.gz      + SHA256
```
...
# Wait ~15-20 minutes
# Release appears at: github.com/agenix-sh/agw/releases
```
...
```bash
cd /path/to/agx
cp -r /path/to/agw/.github .
cp /path/to/agw/rust-toolchain.toml .
...
**For AGW:**
```bash
# Same as above, but AGW → AGW, agw → agw
```
...
### 1. Check PR #11 (AGW-001)
- [ ] Go to: https://github.com/agenix-sh/agw/pull/11
